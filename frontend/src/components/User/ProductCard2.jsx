...
const ProductCard2 = ({ data }) => {
    ...
    return (
        <div className="w-full p-1">
            <div className="sm:w-auto w-[150px] h-[320px] sm:h-[280px] rounded-lg shadow-md relative cursor-pointer border border-black/20 hover:scale-105 transform transition-transform duration-300 ease-in-out">
                <Link to={`/product/2/${data._id}`}>
                    <img src={imageUrl} alt='' className="sm:w-full w-[150px] h-[150px] sm:h-[160px] object-cover" />
                    <h5 className="font-extrabold text-sm px-1 sm:px-3 mt-2">
                        <div className="text-[#f63b60] underline">
                            {data.shop && data.shop.shopName}
                        </div>
                    </h5>
                    <h4 className=" px-1 sm:px-3 text-sm sm:text-md">
